...
#define bitset_setbit(name, index, bit)                                                           \
    ...
    if(bit) {                                                                                     \
        ...
        name[((index - 1) / BIT_AMOUNT) + 1] |= (1LU << (index % BIT_AMOUNT));                    \
    }                                                                                             \
    else {                                                                                        \
...
inline void bitset_setbit(bitset_t name, bitset_index_t index, bitset_index_t bit) {
    if(index < 0 || index > bitset_size(name)) {
        error_exit("bitset_setbit: Index %lu mimo rozsah 0..%lu", index, bitset_size(name));
    }
    if(bit < 0) {
        error_exit("ERROR: bitset_setbit: Invalid bit value\n", index, bitset_size(name));
    }
    if(bit) {
        name[((index - 1) / BIT_AMOUNT) + 1] |= (1LU << (index % BIT_AMOUNT));
    }
